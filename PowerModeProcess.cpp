...
int PowerModeProcess::rawDrawTime = 60; //仮の値
...
void PowerModeProcess::Draw() {
	if (drawTime > 0) { //描写時間が0超過時のみ実行
		...
		for (int i = 0; i < enemy->size(); ++i) {
			if ((*enemy)[i]->GetState() != EnemyAra::State::damage) { (*enemy)[i]->SetRunUpdate(false); } //damage状態以外の敵のUpdateを常に止めるようにする事でやられ状態の敵が巣に着いてready状態になったらUpdateを停止する
